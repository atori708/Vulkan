...
#pragma region ï®óùÉfÉoÉCÉXÇÃçÏê¨
    /// <summary>
    /// ï®óùÉfÉoÉCÉXÇëIëÇ∑ÇÈ
    /// </summary>
    ...
    /// <summary>
    /// ïKóvÇ»ägí£ÇÉTÉ|Å[ÉgÇµÇƒÇ¢ÇÈÇ©ÇämîFÇ∑ÇÈ
    /// </summary>
    ...
    QueueFamilyIndices findGraphicsQueueFamilies(VkPhysicalDevice physicalDevice) {
        ...
        for (const auto& queueFamily : queueFamilies) {
            // ÉOÉâÉtÉBÉNÉXÇÉTÉ|Å[ÉgÇµÇƒÇ¢ÇÈÇ©
            if (queueFamily.queueFlags & VK_QUEUE_GRAPHICS_BIT) {
                indices.graphicsFamily = i;
            }
            
            // ÉvÉåÉ[ÉìÉeÅ[ÉVÉáÉìÇÉTÉ|Å[ÉgÇµÇƒÇ¢ÇÈÇ©
...
#pragma region ò_óùÉfÉoÉCÉXÇÃçÏê¨
    /// <summary>
    /// ò_óùÉfÉoÉCÉXÇçÏê¨Ç∑ÇÈ
    /// </summary>
    VkDevice createLogicalDevice(VkPhysicalDevice physicalDevice, QueueFamilyIndices queueFamilyIndices)
    {
        ...
        if(enableValidationLayers) {
            // å√Ç¢é¿ëïÇæÇ™å›ä∑ê´ÇÃÇΩÇﬂÇ…ì¸ÇÍÇƒÇ®Ç≠
            // ò_óùÉfÉoÉCÉXÇÃåüèÿÉåÉCÉÑÅ[Ç∆ÉCÉìÉXÉ^ÉìÉXÇÃåüèÿÉåÉCÉÑÅ[ÇÕï ÅXÇæÇ¡ÇΩÇÁÇµÇ¢
            createInfo.enabledLayerCount = static_cast<uint32_t>(validationLayers.size());
            createInfo.ppEnabledLayerNames = validationLayers.data();
        }
        else {
...
#pragma region ÉXÉèÉbÉvÉ`ÉFÉCÉì
    /// <summary>
    /// SwapChainÇçÏê¨Ç∑ÇÈ
    /// </summary>
    ...
    /// <summary>
    /// ÉXÉèÉbÉvÉ`ÉFÉCÉìÇÃÉCÉÅÅ[ÉWÉrÉÖÅ[ÇçÏê¨Ç∑ÇÈ
    /// </summary>
    std::vector<VkImageView> createImageViews(std::vector<VkImage> swapChainImages)
    {
        ...
        for (size_t i = 0; i < swapChainImageViews.size(); i++)
        {
            ...
            // ñºëOÇ¬ÇØÇÊÇ§Ç∆ÇµÇΩÇØÇ«ÉäÉìÉNÉGÉâÅ[Ç™èoÇÈ
    ...
    {
        // ÔøΩEÔøΩBÔøΩÔøΩÔøΩhÔøΩEÔøΩÔøΩÔøΩ≈èÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩÍÇΩÔøΩ∆ÇÔøΩÔøΩ…ïÔøΩÔøΩ∆çÔøΩÔøΩÔøΩÔøΩÔøΩ0ÔøΩ…Ç»ÇÔøΩÃÇ≈ÅAÔøΩÔøΩÔøΩÃèÍçáÔøΩÕÉÔøΩÔøΩTÔøΩCÔøΩYÔøΩÔøΩÔøΩÔøΩÔøΩ‹Ç≈ë“ã@ÔøΩÔøΩÔøΩÔøΩ
...
#pragma endregion
    ...
    VkRenderPass createRenderPass(VkFormat swapChainImageFormat)
    {
        ...
        // ÉTÉuÉpÉXä‘ÇÃàÀë∂ä÷åWÇÃê›íË(ç°âÒÇÕ1Ç¬ÇµÇ©ÉTÉuÉpÉXÇ™Ç»Ç¢ÇÃÇ≈è»ó™ÇµÇƒÇ‡ÇÊÇ¢)
...
#pragma region ÉOÉâÉtÉBÉNÉXÉpÉCÉvÉâÉCÉìÇÃçÏê¨
    VkPipeline createGraphicsPipeline(VkRenderPass renderPass, VkShaderModule vertShaderModule, VkShaderModule fragShaderModule)
    {
        ...
        // Dynamic State
        // PSOÇÕäÓñ{ìIÇ…ïsïœÇæÇ™ÅAàÍïîÇÕìÆìIÇ…ïœçXÇ≈Ç´ÇÈÇÃÇ≈ÇªÇÃê›íË
        // Ç±ÇÍÇÇ∑ÇÈÇ±Ç∆Ç≈ÅAï`âÊéûÇ…ÉfÅ[É^ÇéwíËÇ≈Ç´ÇÈÇÊÇ§Ç…Ç»ÇÈ(éwíËÇ™ïKê{Ç…Ç»ÇÈ)
        ...
        inputAssembly.primitiveRestartEnable = VK_FALSE; // VK_TRUEÇ…Ç∑ÇÈÇ∆ÅAtopologyÇSTRIPÇ…ÇµÇΩÇ∆Ç´ÇÃLINEÇ∆É|ÉäÉSÉìÇì¡éÍÇ»ÉCÉìÉfÉbÉNÉXílÇ≈ÉvÉäÉ~ÉeÉBÉuÇï™äÑÇ≈Ç´ÇÈ...ÅHÇÁÇµÇ¢
        ...
        rasterizerCreateInfo.depthClampEnable = VK_FALSE; // VK_TRUEÇ…Ç∑ÇÈÇ∆ÅAÉNÉäÉbÉvãÛä‘ÇÃäOÇ…èoÇΩê[ìxÇÉNÉâÉìÉvÇ∑ÇÈ(îjä¸ÇÕÇµÇ»Ç¢)ÅBÉVÉÉÉhÉEÉ}ÉbÉvÇ»Ç«ÇÃÉPÅ[ÉXÇ≈ñÇ…óßÇ¬ÇÁÇµÇ¢
        rasterizerCreateInfo.rasterizerDiscardEnable = VK_FALSE; // VK_TRUEÇ…Ç∑ÇÈÇ∆ÅAÉtÉâÉOÉÅÉìÉgÉVÉFÅ[É_Ç‹Ç≈çsÇ©Ç∏Ç…ÉvÉäÉ~ÉeÉBÉuÇîjä¸Ç∑ÇÈÅBí∏ì_èàóùÇæÇØÇµÇΩÇ¢èÍçáÇ»Ç«Ç…égÇ§
        rasterizerCreateInfo.polygonMode = VK_POLYGON_MODE_FILL; // É|ÉäÉSÉìÇÃï`âÊÉÇÅ[ÉhÅBLINEÇ‚POINTÇ‡Ç†ÇÈ
        ...
        // ê[ìxÉXÉeÉìÉVÉãÇÃê›íËÇ‡Ç†ÇÈÇØÇ«ç°ÇÕÇ¢ÇÁÇ»Ç¢ÇÃÇ≈è»ó™
...
#pragma region ÉRÉ}ÉìÉhÉvÅ[ÉãÇ∆ÉRÉ}ÉìÉhÉoÉbÉtÉ@
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, VkRenderPass renderPass, uint32_t swapChainBufferIndex, VkExtent2D swapChainExtent)
    {
...
#pragma endregion
    ...
    /// <summary>
    /// ÉtÉ@ÉCÉãÇÉoÉCÉiÉäå`éÆÇ≈ÉçÅ[ÉhÇµÇ‹Ç∑
    /// </summary>
