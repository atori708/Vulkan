...
private:
    ...
    void drawFrame()
    {
        ...
        recordCommandBuffer(commandBuffer, renderPass, imageIndex, swapChainExtent);
...
#pragma region 論理デバイスの作成
    ...
    VkDevice createLogicalDevice(VkPhysicalDevice physicalDevice, QueueFamilyIndices queueFamilyIndices)
    {
        ...
        if(enableValidationLayers) {
            // 古い実装だが互換性のために入れておく
            // 論理デバイスの検証レイヤーとインスタンスの検証レイヤーは別々だったらしい
            createInfo.enabledLayerCount = static_cast<uint32_t>(validationLayers.size());
            createInfo.ppEnabledLayerNames = validationLayers.data();
        }
        else {
...
#pragma endregion
    ...
    VkRenderPass createRenderPass(VkFormat swapChainImageFormat)
    {
        ...
        // サブパス間の依存関係の設定(今回は1つしかサブパスがないので省略してもよい)
...
#pragma region グラフィクスパイプラインの作成
    VkPipeline createGraphicsPipeline(VkRenderPass renderPass, VkShaderModule vertShaderModule, VkShaderModule fragShaderModule)
    {
        ...
        // Dynamic State
        // PSOは基本的に不変だが、一部は動的に変更できるのでその設定
        // これをすることで、描画時にデータを指定できるようになる(指定が必須になる)
        ...
        inputAssembly.primitiveRestartEnable = VK_FALSE; // VK_TRUEにすると、topologyをSTRIPにしたときのLINEとポリゴンを特殊なインデックス値でプリミティブを分割できる...？らしい
        ...
        rasterizerCreateInfo.depthClampEnable = VK_FALSE; // VK_TRUEにすると、クリップ空間の外に出た深度をクランプする(破棄はしない)。シャドウマップなどのケースで役に立つらしい
        rasterizerCreateInfo.rasterizerDiscardEnable = VK_FALSE; // VK_TRUEにすると、フラグメントシェーダまで行かずにプリミティブを破棄する。頂点処理だけしたい場合などに使う
        rasterizerCreateInfo.polygonMode = VK_POLYGON_MODE_FILL; // ポリゴンの描画モード。LINEやPOINTもある
...
#pragma region コマンドプールとコマンドバッファ
